...
# --- SYSTEM PROMPT ---
# This constant defines the "personality" and rules for the AI.
# It instructs the AI to roleplay as specific characters in a sci-fi setting.
...
# Retrieve the Deepgram API key for Text-to-Speech (TTS)
...
# Map character names (e.g., PlayerMale1) to specific Deepgram voice models (e.g., aura-2-odysseus-en)
MODEL_MAP = {
...
def parse_dialogues(text):
    """
    Parses the AI's response text to extract character names and their dialogue.
    Uses regex to find patterns like <PlayerMale1>Hello</PlayerMale1>.
    Returns a list of tuples: [('PlayerMale1', 'Hello'), ...]
    """
...
def process_dialogue_text(dialogue_text):
    ...
    # Play the downloaded audio files in the correct order
    # (Text + Audio sync happens here conceptually, though this script just plays audio)
...
def groq_chat(prompt):
    """
    Sends the constructed prompt to the Groq API (acting as the LLM).
    Returns the text response generated by the AI.
    """
